...
use MagicDocs\Parsers\ParserInterface;
...
class Documentor
{
    ...
    public static function readConfig(): array
    {
        if (static::$config === null) {
            if (static::$faked === false) {
                static::$config = \Illuminate\Support\Facades\Config::get('magicdocs');
            }

            static::$config ??= require __DIR__ . '/../config/magicdocs.php';
        }
    ...
    {
        return static::getPath(static::readConfig()['output'] ?? 'magicdocs_generated.php');
    }
    ...
    {
        $docs = new Docs();
        $magicdocsConfig = static::readConfig();

        foreach ($magicdocsConfig['directories'] ?? [] as $directory => $data) {
        ...
        foreach ($magicdocsConfig['standalones'] ?? [] as $parser => $config) {
            // Disable parser if value is false, e.g: "My\ParserClass::class => false"
            if ($config === false) {
                continue;
            }
