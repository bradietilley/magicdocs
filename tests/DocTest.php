...
test('can create a doc type for an enum property', function () {
    $doc = new Doc(
        ...
        type: 'property',
        name: 'thing',
        schemaType: TypeMultiple::parse('MagicDocs\SomeEnum'),
        schemaDefault: VariableString::parse(\MagicDocs\SomeEnum::BB),
    );

    expect($doc->compile())->toBe('@property \MagicDocs\SomeEnum $thing = \MagicDocs\SomeEnum::BB');
});
...
test('can create a doc type for an enum method', function () {
    $doc = new Doc(
        ...
        type: 'method',
        name: 'getThing',
        schemaReturn: TypeMultiple::parse('MagicDocs\SomeEnum'),
    );

    expect($doc->compile())->toBe('@method \MagicDocs\SomeEnum getThing()');
});
...
test('can create a doc type for a method with arguments', function () {
    $doc = new Doc(
        ...
        schemaArgs: new ArgumentList([
            ...
            new Argument(
                'enum',
                TypeMultiple::parse(\MagicDocs\SomeEnum::class),
                VariableString::parse(\MagicDocs\SomeEnum::BB),
            ),
        ]),
    );

    expect($doc->compile())->toBe('@method array getThings(\App\Models\User $user = null, \MagicDocs\SomeEnum $enum = \MagicDocs\SomeEnum::BB)');
});
